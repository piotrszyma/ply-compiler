...
_lr_productions = [
  ...
  ('program -> VAR vdeclarations BEGIN commands END','program',5,'p_program','parser.py',27),
  ('vdeclarations -> vdeclarations PIDENTIFIER','vdeclarations',2,'p_vdeclarations_number','parser.py',33),
  ('vdeclarations -> vdeclarations PIDENTIFIER LBRACKET NUMBER RBRACKET','vdeclarations',5,'p_vdeclarations_table','parser.py',38),
  ('vdeclarations -> empty','vdeclarations',1,'p_vdeclarations_empty','parser.py',43),
  ('commands -> commands command','commands',2,'p_commands','parser.py',49),
  ('commands -> command','commands',1,'p_commands','parser.py',50),
  ('command -> identifier ASSIGN expression SEMICOLON','command',4,'p_command_assign','parser.py',60),
  ('command -> IF condition THEN commands ELSE commands ENDIF','command',7,'p_command_if_else','parser.py',64),
  ('command -> IF condition THEN commands ENDIF','command',5,'p_command_if_then','parser.py',68),
  ('command -> WHILE condition DO commands ENDWHILE','command',5,'p_command_while','parser.py',72),
  ('command -> FOR PIDENTIFIER FROM value TO value DO commands ENDFOR','command',9,'p_command_for_up','parser.py',76),
  ('command -> FOR PIDENTIFIER FROM value DOWNTO value DO commands ENDFOR','command',9,'p_command_for_down','parser.py',81),
  ('command -> READ identifier SEMICOLON','command',3,'p_command_read','parser.py',86),
  ('command -> WRITE value SEMICOLON','command',3,'p_command_write','parser.py',90),
  ('expression -> value','expression',1,'p_expression_value','parser.py',96),
  ('expression -> value PLUS value','expression',3,'p_expression','parser.py',100),
  ('expression -> value MINUS value','expression',3,'p_expression','parser.py',101),
  ('expression -> value TIMES value','expression',3,'p_expression','parser.py',102),
  ('expression -> value DIVIDE value','expression',3,'p_expression','parser.py',103),
  ('expression -> value MODULO value','expression',3,'p_expression','parser.py',104),
  ('condition -> value EQ value','condition',3,'p_condition','parser.py',110),
  ('condition -> value NE value','condition',3,'p_condition','parser.py',111),
  ('condition -> value LT value','condition',3,'p_condition','parser.py',112),
  ('condition -> value GT value','condition',3,'p_condition','parser.py',113),
  ('condition -> value LE value','condition',3,'p_condition','parser.py',114),
  ('condition -> value GE value','condition',3,'p_condition','parser.py',115),
  ('value -> NUMBER','value',1,'p_value','parser.py',121),
  ('value -> identifier','value',1,'p_value','parser.py',122),
  ('identifier -> PIDENTIFIER','identifier',1,'p_identifier_pidentifier','parser.py',128),
  ('identifier -> PIDENTIFIER LBRACKET PIDENTIFIER RBRACKET','identifier',4,'p_identifier_table_id','parser.py',132),
  ('identifier -> PIDENTIFIER LBRACKET NUMBER RBRACKET','identifier',4,'p_identifier_table_number','parser.py',137),
