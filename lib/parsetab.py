...
_lr_productions = [
  ...
  ('program -> VAR vdeclarations BEGIN commands END','program',5,'p_program','parser.py',25),
  ('vdeclarations -> vdeclarations PIDENTIFIER','vdeclarations',2,'p_vdeclarations_number','parser.py',31),
  ('vdeclarations -> vdeclarations PIDENTIFIER LBRACKET NUMBER RBRACKET','vdeclarations',5,'p_vdeclarations_table','parser.py',36),
  ('vdeclarations -> empty','vdeclarations',1,'p_vdeclarations_empty','parser.py',41),
  ('commands -> commands command','commands',2,'p_commands','parser.py',47),
  ('commands -> command','commands',1,'p_commands','parser.py',48),
  ('command -> identifier ASSIGN expression SEMICOLON','command',4,'p_command_assign','parser.py',58),
  ('command -> IF condition THEN commands ELSE commands ENDIF','command',7,'p_command_if_else','parser.py',62),
  ('command -> IF condition THEN commands ENDIF','command',5,'p_command_if_then','parser.py',66),
  ('command -> WHILE condition DO commands ENDWHILE','command',5,'p_command_while','parser.py',70),
  ('command -> FOR PIDENTIFIER FROM value TO value DO commands ENDFOR','command',9,'p_command_for_up','parser.py',74),
  ('command -> FOR PIDENTIFIER FROM value DOWNTO value DO commands ENDFOR','command',9,'p_command_for_down','parser.py',79),
  ('command -> READ identifier SEMICOLON','command',3,'p_command_read','parser.py',84),
  ('command -> WRITE value SEMICOLON','command',3,'p_command_write','parser.py',88),
  ('expression -> value','expression',1,'p_expression_value','parser.py',94),
  ('expression -> value PLUS value','expression',3,'p_expression','parser.py',98),
  ('expression -> value MINUS value','expression',3,'p_expression','parser.py',99),
  ('expression -> value TIMES value','expression',3,'p_expression','parser.py',100),
  ('expression -> value DIVIDE value','expression',3,'p_expression','parser.py',101),
  ('expression -> value MODULO value','expression',3,'p_expression','parser.py',102),
  ('condition -> value EQ value','condition',3,'p_condition','parser.py',108),
  ('condition -> value NE value','condition',3,'p_condition','parser.py',109),
  ('condition -> value LT value','condition',3,'p_condition','parser.py',110),
  ('condition -> value GT value','condition',3,'p_condition','parser.py',111),
  ('condition -> value LE value','condition',3,'p_condition','parser.py',112),
  ('condition -> value GE value','condition',3,'p_condition','parser.py',113),
  ('value -> NUMBER','value',1,'p_value','parser.py',119),
  ('value -> identifier','value',1,'p_value','parser.py',120),
  ('identifier -> PIDENTIFIER','identifier',1,'p_identifier_pidentifier','parser.py',126),
  ('identifier -> PIDENTIFIER LBRACKET PIDENTIFIER RBRACKET','identifier',4,'p_identifier_table_id','parser.py',130),
  ('identifier -> PIDENTIFIER LBRACKET NUMBER RBRACKET','identifier',4,'p_identifier_table_number','parser.py',135),
